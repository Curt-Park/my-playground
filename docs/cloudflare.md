...
3. Select a plan (Free plan works)
4. Update your domain registrar's nameservers to the ones Cloudflare provides
5. Wait for nameserver propagation (can take up to 24 hours)
...
Two tokens are needed: a scoped **DNS API token** and a **Global API token**.
...
6. Create the token and set it as `CF_DNS_TOKEN` in your `.env`
...
1. Go to **My Profile** -> [**API Tokens**](https://dash.cloudflare.com/profile/api-tokens)
2. Scroll to **Global API Key** and click **View**
3. Copy the key and set it as `CF_GLOBAL_TOKEN` in your `.env`

> **Note:** The Global API Key has full account access. traefik-cloudflare-companion requires it because it does not support scoped tokens. Keep this key secure.
...
This ensures Cloudflare connects to your origin over HTTPS and validates the Let's Encrypt certificate.
